...
public class Utilidades {
	...
	//EJERCICIO 1 EXAMEN 5 
	/**
	 * validar que se pueda leer un float y si es incorrecto volver a pedirle al usuario que introduzca otro numero 
	 * @return
	 */
	public static double leerFloat() {
		float ret = (float) 0.0;
		boolean correcto = false;
		Scanner in;
		do {
			System.out.println("Introduzca un valor float (xx.xx)");
			in = new Scanner(System.in, "ISO-8859-1");
			try {
				ret = in.nextFloat();
				correcto = true;
				...
				System.out.println("Formato introducido incorrecto.");
				correcto = false;
				//coge todas las excepciones que puede tener el leer float
			} catch (Exception e) {
