...
public class DatosPersona {
	...
	public static DatosPersona nuevaPersona() {
		...
		if (esnif) {
			doc = NIF.nuevoNIF();
			String nif = " ";
			boolean validarnif = false;
			do {
				System.out.println("introduce un NIF correcto");
				nif = in.next();
				// aqui iria el validador como en el nie pero tengo un error de validacion con
				// el nif
			} while (!validarnif);
		} else {
			...
			doc = NIE.nuevoNIE();
			String nie = " ";
			boolean validarnie = false;
			do {
				System.out.println("introduce un NIF correcto");
				nie = in.next();
				validarnie = validaciones.Validador.validarNIE(nie);
			} while (!validarnie);
		}
